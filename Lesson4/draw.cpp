...
Vec3f Draw::barycentric(Vec3f* pts, Vec3f P)
{
    ...
    Vec3f yVec = Vec3f( pts[1][1] - pts[0][1],
                        ...
                        pts[0][1] - P.y );
    Vec3f ret = xVec ^ yVec;
    if(std::abs(ret[2]) > 1e-2) 
        return Vec3f(1.0 - (ret.x + ret.y)/ret.z, ret.x/ret.z, ret.y/ret.z);
    return Vec3f(-1, 1, 1);
}

void Draw::triangle(Vec3f* pts, Vec2f* uvs, float* zbuffer, TGAImage& image, Texture* texture)
{
    //找到包围盒
    ...
    for(P.x = boxmin.x; P.x <= boxmax.x; P.x++){
        for(P.y = boxmin.y; P.y <= boxmax.y; P.y++){
            ...
            Vec2f point(0.0, 0.0);
            for(int i = 0; i < 3; i++){
                P.z += pts[i][2] * screen[i];
                point.x += uvs[i].x * screen[i];
                point.y += uvs[i].y * screen[i];
            ...
            if(zbuffer[int(P.x + P.y * image.get_width())] <= P.z){
                ...
                TGAColor color = texture->get_diffuse_color(point);
